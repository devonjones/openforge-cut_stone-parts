...
y = 1;
external = false;
render_cut_stone_curved_base = true;
    ...
    union() {
        ...
        intersection() {
            difference() {
                ...
                color("LightGrey") translate([0,0,-1]) scale([x < y ? ((25*x)-1)/((25*y)-1) : 1, y < x ? ((25*y)-1)/((25*x)-1) : 1,1]) cylinder(10,max([x, y])*25-1, max([x,y])*25-1,$fn=100);
            }
            color("LightGrey") translate([1,1,0]) cube([25*x,25*y,5]);
        }
    }
}
